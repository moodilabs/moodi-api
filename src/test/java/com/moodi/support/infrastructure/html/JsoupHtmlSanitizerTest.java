...
class JsoupHtmlSanitizerTest {
    ...
    @Test
    @DisplayName("굵게·글자색·형광펜·줄바꿈·목록 같은 에디터 서식은 남긴다")
    void keeps_editor_formatting() {
        String html = "<p><strong>제1조</strong> 목적<br><span style=\"color: #ff0000\">중요</span><mark style=\"background-color: #fef08a\">강조</mark></p><ul><li>항목</li></ul>";

        String result = sanitizer.sanitize(html);

        assertThat(result).contains("<strong>제1조</strong>", "<br>", "<span style=\"color: #ff0000\">중요</span>",
                "<mark style=\"background-color: #fef08a\">강조</mark>", "<li>항목</li>");
    }
    ...
    void strips_dangerous_markup() {
        String html = "<p onclick=\"alert(1)\">본문</p><script>alert(1)</script>"
                + "<span style=\"position:fixed;color:red\">x</span><mark style=\"background-image:url(x)\">y</mark><img src=\"http://evil/x.png\">";

        String result = sanitizer.sanitize(html);

        assertThat(result).doesNotContain("script", "onclick", "position", "img", "url(");
        assertThat(result).contains("<p>본문</p>", "<span>x</span>", "<mark>y</mark>");
    }
