...
##### 노출 이력 유효 기간 — **30일** (2026-08-03 확정)

`shown_at > now() - 30일` 인 것만 "본 것"으로 친다. 그보다 오래되면 다시 "안 본 것"으로 돌아온다.

**새로고침 동작과 혼동하지 말 것.** 새로고침하면 유효 기간과 무관하게 매번 피드가 새로 구성된다
(새 seed + 방금 본 것 후순위). 유효 기간이 정하는 것은 **한 번 본 스팟을 언제 다시 앞으로 올릴까**,
즉 재노출 주기다.

값을 30일로 잡은 이유는 **스팟 카탈로그가 크기 때문**이다. 카탈로그 규모에 따라 이 설정의 역할이 다르다.

| | 카탈로그 작음 | **카탈로그 큼 (현재)** |
|---|---|---|
| 주 목적 | 재순환 — 다 본 뒤 다시 돌리기 | **최근에 본 것 재등장 방지** |
| 적정값 | 짧게 (7일) | **길게 (30일)** |

카탈로그가 크면 사용자가 매칭 스팟을 다 볼 일이 없으므로 재순환 압박이 없다.
짧게 잡을 이유가 없고, 길게 잡을수록 "아까 본 게 또 뜬다"가 줄어든다.

설정값으로 빼서 운영 중 조정 가능하게 한다.
...
- **피드 쿼리 성능** — 스팟 카탈로그가 크다. 무드 교집합(`jsonb ?|`) + `feed_impression` 조인 +
  `ORDER BY hash` + keyset 페이징이 **매 페이지마다** 돈다. 스팟 상세의 "유사 무드 5개"(단발 조회)와
  부하 성격이 다르다. **이슈 #21(JSONB GIN 인덱스 기반 무드 필터링 성능 검증)이 여기에 직결되므로
  Feed 구현과 함께 인덱스 설계를 확인해야 한다.** GIN 인덱스 없이 가면 피드가 첫 병목이 될 가능성이 높다.
  `feed_impression`도 `(member_id, shown_at)` 인덱스가 필요하다.
