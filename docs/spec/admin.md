...
| `POST` | `/api/admin/policies` | `{ type, version, content, effectiveAt }` → 201. `(type, version)` 중복 → `POLICY_VERSION_DUPLICATE` 409. `content`는 에디터 HTML — 서버가 jsoup 화이트리스트(p/br/strong/em/u/s/h1~h3/ul/ol/li/span color/mark background-color/a/table 계열(th·td colspan·rowspan))로 정제 |
