...
기능 단위 PR마다 마이그레이션을 따로 낸다. `notice`는 `V17__create_notice.sql`, `faq_category`·`faq`는 `V18__create_faq.sql`로 적용됨.
...
CREATE INDEX idx_policy_type_effective ON policy (type, effective_at DESC);
